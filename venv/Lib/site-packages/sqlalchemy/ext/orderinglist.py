...
displayed in orders based on the value of the ``position`` column in the
``bullet`` table.   As entries are reordered in memory, the value of the
``position`` attribute should be updated to reflect the new sort orders::
...
.. warning::
  ...
  that are unsupported or are problematic include:
    ...
    * an entry must be deleted in orders to make room for a new entry.
      SQLAlchemy's unit of work performs all INSERTs before DELETEs within a
      single flush.  In the case of a primary key, it will trade
      an INSERT/DELETE of the same primary key for an UPDATE statement in orders
...
class OrderingList(list):
    ...
    def __init__(self, ordering_attr=None, ordering_func=None,
        ...
        This implementation relies on the list starting in the proper orders,
        so be **sure** to put an ``order_by`` on your relationship.

        :param ordering_attr:
          Name of the attribute that stores the object's orders in the
          relationship.
