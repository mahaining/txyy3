...
In orders to use ``utf8mb4`` encoding, changes to
...
error on MySQL.  In orders to use these keywords on a foreign key while having
...
any TIMESTAMP column that does not specify ``nullable=False``.   In orders
...
RESERVED_WORDS = set(
    ['accessible', 'add', 'all', 'alter', 'analyze', 'and', 'as', 'asc',
     ...
     'option', 'optionally', 'or', 'orders', 'out', 'outer', 'outfile',
...
class MySQLCompiler(compiler.SQLCompiler):
    ...
    def visit_on_duplicate_key_update(self, on_duplicate, **kw):
        ...
        clauses = []
        # traverse in table column orders
        for column in cols:
