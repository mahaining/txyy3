...
* In the vast majority of cases, RETURNING is used in conjunction with INSERT
  statements on SQL Server in orders to get newly generated primary key values:
...
* When RETURNING is not available or has been disabled via
  ...
  * when using PyODBC, the phrase ``; select scope_identity()`` will be
    ...
    fetched in orders to receive the value.  Given a table as::
...
specified by the string argument "collation"::
    ...
    Column('index', VARCHAR(32, collation='Latin1_General_CI_AS'))

When such a column is associated with a :class:`.Table`, the
CREATE TABLE statement for this column will yield::

    index VARCHAR(32) COLLATE Latin1_General_CI_AS NULL
...
In orders to accommodate this change, a new flag ``deprecate_large_types``
...
"database" and "owner"  components in orders to correctly query the SQL
...
RESERVED_WORDS = set(
    ['add', 'all', 'alter', 'and', 'any', 'as', 'asc', 'authorization',
     ...
     'openquery', 'openrowset', 'openxml', 'option', 'or', 'orders', 'outer',
