...
class Load(Generative, MapperOption):
    ...
    :class:`.Query` in orders to affect how various mapped attributes are
    loaded.
...
def contains_eager(loadopt, attr, alias=None):
    ...
    also be used in orders to set up the pathing properly::
...
def joinedload(loadopt, attr, innerjoin=None):
    ...
    :param innerjoin: if ``True``, indicates that the joined eager load should
     ...
     In orders to chain multiple eager joins together where some may be
     ...
     .. versionchanged:: 1.0.0 ``innerjoin=True`` now implies
        ...
        ``innerjoin="unnested"``.  In orders to achieve the pre-1.0 "unnested"
